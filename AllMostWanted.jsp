...
<%@page import="crimeManagementSystem.MostWanted,crimeManagementSystem.MostWantedDao.*,java.util.*"%>  
...
<h3><b>Most Wanted Criminals</b></h3>
<hr>
</body>

<%  
List<MostWanted> list=MostWantedDao.getAllRecords();  
...
<tr><th>Id</th><th>policestationname</th><th>image</th><th>fullname</th> 
 <th>nickname</th><th>idproof</th><th>gender</th><th>dateofbirth</th><th>address</th><th>identifying mark</th><th>description</th>
<c:forEach items="${list}" var="u">  
<tr><td>${u.getId()}</td><td>${u.getPolicestationname()}</td><td>${u.getImage()}</td><td>${u.getFullname()}</td>
<td>${u.getNickname()}</td><td>${u.getIdproof()}</td><td>${u.getGender()}</td><td>${u.getDateofbirth()}</td><td>${u.getAddress()}</td>
<td>${u.getIdentifyingmark()}</td><td>${u.getDescription()}</td>
</tr>
