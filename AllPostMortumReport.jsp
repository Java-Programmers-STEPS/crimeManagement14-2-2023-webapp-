...
<%@page import="crimeManagementSystem.PostMortumDao,crimeManagementSystem.PostMortum.*,java.util.*"%>  
...
<h3><b>Post-Mortum Reports</b></h3>
<hr>
</body>

<%  
List<PostMortum> list=PostMortumDao.getAllRecords();  
...
<tr><th>Id</th><th>Full name</th><th>Image</th><th>Gender</th>
<th>Id proof</th><th>Address</th><th>FIR Id</th><th>Post-Mortum Report</th><th>Doctor's Name</th><th>Hospital Address</th><th>Description of Case</th></tr> 
<c:forEach items="${list}" var="u">
<tr><td>${u.getId()}</td><td>${u.getFullname()}</td><td>${u.getImage()}</td><td>${u.getGender()}</td>
<td>${u.getIdproof()}</td> <td>${u.getAddress()}</td><td>${u.getFirid()}</td><td>${u.getPostmortum()}</td><td>${u.getDoctorsname()}</td><td>${u.getHospitaladdress()}</td>
<td>${u.getDescriptionofcase()}</td></tr>
